...
argument-hint: (no arguments — reads $gate, $check-protected-paths, $behavioral-validation-p1, $behavioral-e2e-p1, $security-check-p1, $code-review-p1, $fetch-base-governance)
---
...
pytest suite. There are no separate static-checks or run-tests nodes any more:
they duplicated these two rungs and read the worktree while the gate's mutation
rung was rewriting files in place, so they reported defects the judge injected.

### Protected Paths and PR Size (deterministic — FACTORY_RULES §5 and §8)
$check-protected-paths.output

`PROTECTED_CLEAN` plus `SIZE_OK` means the PR changed nothing on
`.factory/protected-paths.txt` and stayed inside 500 lines.
`PROTECTED_TOUCHED path=...` or `SIZE_EXCEEDED` is a hard REJECT.
`PROTECTED_PARTIAL path=...` is not a verdict: it names a file whose invariant
lives in part of it, and tells you to weigh the security and code reviewers'
reading of that diff rather than reject on the filename alone.
...
Additionally, if `$gate.output` or `$behavioral-e2e-p1.output` is empty (no content at all — meaning the node was skipped because its upstream dependency failed), that is also an infrastructure failure even if `checkout-pr.output` looks fine.
...
8. `$gate.output` contains `GATE_FAILED: holdout`, `GATE_FAILED: mutations`, or a line matching `RATCHET_FAILED floors=` / `  RATCHET_FAIL ` — the holdout the builder cannot read, the mutation set, or a ratchet floor went red. Either a MISSION invariant broke or the judge itself was touched. Always `should_escalate: true`.

   **Match those exact forms, not the bare substring `RATCHET_FAILED`.** `harness/ratchet.py` prints `RATCHET_FAILED floors=N failures=M` (with a `  RATCHET_FAIL <floor>` line per failure) for a genuine floor violation, and that is the only form that belongs here. `RATCHET_SKIPPED gate_red` means the gate was red so the ratchet was never run against it; that is a build failure, not a lowered floor, and it belongs in REQUEST_CHANGES below. Treating any red build as a floor violation sent every ruff error and failing test to a human and made the fix pass unreachable.
...
9. `$check-protected-paths.output` contains `PROTECTED_TOUCHED path=...` or `SIZE_EXCEEDED`, or lacks both `PROTECTED_CLEAN` and `SIZE_OK` — the PR changed a file on `.factory/protected-paths.txt`, blew the 500-line cap, or the check could not run. FACTORY_RULES §5 and §6.1: close the PR, and per §7 escalate the issue too, because a factory PR reaching for the judge or the constitution means the scope was misunderstood. Always `should_escalate: true`. (apply-verdict enforces this in bash as well; you should reach the same answer.)
...
2. Static checks: `$gate.output` contains `STATIC_OK`
3. Service tests: `$gate.output` contains `UNIT_PASSED tests=N` with N > 0. There is no legitimate skip: the suite exists and every PR must keep it green
...
- `$gate.output` contains `GATE_FAILED: static`, `GATE_FAILED: unit` or `GATE_FAILED: e2e` — the build is red in a rung a fix workflow can address. `RATCHET_SKIPPED gate_red` accompanies these and changes nothing: it only says the ratchet had no green log to measure
